...
- **The one catch to see (cluster c2).** Its textbook stromal markers COL1A1 and VIM are **off-panel**, never measured. A general reader (or a general LLM) counts that as evidence against stroma; Panoscope checks the panel first, marks them "not measured" rather than "absent", and holds the call on LUM (glm 18.00) and POSTN (glm 15.80). An absence you never measured is not evidence.
- **Why trust it.** Nothing is fabricated: a grounding gate discards any answer whose number, marker, or PMID does not trace to source. Check it yourself from a fresh clone, no API key: `pytest -m "not live"` (230+ tests; a green run *is* the guarantee).
- **Depth.** Two skill-driven workflows (marker cell-typing + gene-set enrichment), a scope-enforced memory layer that records the biologist's overrides with the literature tension visible, a live rival-test, and three linked spatial views. Built with Claude Code as a team of agents against one contract.
- **Where to look.** the grounding gate [`agent/grounding_check.py`](agent/grounding_check.py), the deterministic verdict engine [`agent/verdict.py`](agent/verdict.py), the portable skill [`skills/jazzpanda-markers/SKILL.md`](skills/jazzpanda-markers/SKILL.md), and the calibration table (`python scripts/calibration_table.py`).
...
5. **Memory as scope-enforced lab notes, plus a portable user memory.** When the biologist's judgment diverges from the default, Panoscope captures it as one of **eight typed, anchored notes** (`{ claim, type, scope, basis, status, tension }`, git-tracked). Not just a full cell-type override, but the everyday moments that used to evaporate: a marker re-read (*"POSTN here is tumor bleed, down-weight it"*), a re-interpreted pathway program, a validation (*"we confirmed c4 by p63 IHC"*), a confidence adjustment, an exclude, or a *"these two clusters are one population"*. Capture is one lightweight tap: confirm the **basis** (scope is inferred, since Panoscope works one dataset at a time), and the claim is cross-checked against the literature *before* it renders, keeping the biologist's call *with the disagreement visible*, from all three surfaces (the marker chat, the Pathways chat, and the holistic review). Notes render **anchored to their subject**: a marker note as a caveat row beneath that gene's driver, a program note beside its gene-set row, a confidence note as a dual band (`lab: High · computed Medium`, never overwriting the computed value). Scope is fail-closed (a cluster- or dataset-scoped note never fires elsewhere) and the agent must cite a note (`[note:id]`) to use it. An `exclude` note flips the exported flag at report-composition time **without mutating the deterministic verdict underneath**, and no note can ever rewrite a jazzPanda number. A decision can also be saved to a **portable user memory** (`agent/user_memory.py`): a distilled, tissue-tagged copy kept locally across projects (`context/user_memory.jsonl`, git-ignored) and loaded back as prior lab knowledge on future datasets, still open-ceiling and unable to move a number. It is a knowledge layer the lab owns; nothing is trained, nothing learns on its own.
6. **"What would settle it" (test a rival cell type).** When a call is ambiguous (the biologist asks *"could c1 be myoepithelial?"*, or a cluster is flagged `verify`), the agent names the concrete discriminating markers (`agent/discriminate.py`), grounded in that cluster's *own* jazzPanda numbers. The biologist triggers this directly with **"Test another cell type,"** a dedicated input beside the chat where they name a rival call and the agent weighs it against the current one, kept distinct from open Ask chat. Because jazzPanda assigns each gene to a single winning cluster, a marker's coefficient exists only there, so the discriminator re-reads a cluster's own markers across cell types: markers that support the call here (with numbers), alternative-type markers that are on the panel but localize to another cluster (measured, so they argue *against* the alternative, e.g. ACTA2/MYH11 localize to c4, not c1), and alternative-type markers that are off-panel (TP63), **flagged** as never-measured, never a bench recommendation. Deterministic with a demo-safe fallback; the live agent adds one real citation for the distinguishing claim. Asserted in `tests/test_discriminate.py`.
7. **A downloadable interpretation summary.** The Summary page auto-assembles a per-dataset report from the durable, grounded artifacts, structured like the skill's Output 1: a header (tissue, platform, panel, method), a **confirmed-call overview table** (cluster, cell type, lineage, confidence, top markers), then per-cluster detail (the call and its driving numbers, the live-cited biology, the enriched **gene-set programs** read panel-scoped, the "what would settle it" line for shaky calls, and your saved lab notes with their tension), then the cross-cluster review and panel caveats. Reviewable on the page and downloadable as Word (`.docx`) or PDF (`ui/report.py`). There is no live network at export time, so the report is a reproducible artifact. Asserted in `tests/test_report.py`.
8. **A second opinion, self-check separated from rival-test.** The deterministic **risk flag** (`agent/skeptic.py`) screens every call from the numbers alone (evidence thinness, a driver that localizes as well elsewhere, a rival whose on-panel markers also peak here) and reads *withstands* or *re-check* instantly; it auto-shows on a `verify`-flagged call as a triage light. It is a grounded rule, not an agent, and it is named as one. The complementary move, testing a *specific* competing cell type, is the chat action in item 6: **"Test another cell type"** seeds the biologist's alternative, and the agent weighs it against the current call through the same grounded discriminator and the same grounding gate. A live adversarial variant (`loop.pressure_test`, a careful skeptic contract that never treats an off-panel absence as evidence, invents a rival with no on-panel signal, or manufactures doubt on a clean call) is retained and tested in `tests/test_skeptic.py`.
9. **Tissue-aware, and tuned to the lab, kept private.** Every prompt carries a grounded **dataset context** read from the manifest (tissue and platform, e.g. *human breast cancer, Xenium*), so interpretation and literature search are tissue-appropriate. Tissue is a search *preference, not a filter*: a marker's canonical biology is often established in another tissue, so a real cross-tissue paper is a valid citation and a tissue mismatch is disclosed, never invented around. On top of that, the biologist can note a one-line personal focus at setup (e.g. *CAF heterogeneity*); it is stored locally and sharpens which real paper is cited. Neither the tissue context nor the research focus can ever change a jazzPanda number, a marker, or a confidence, only which real paper is surfaced.
...
The skill is the interpretation contract; the **per-dataset pipeline is its executor**. `python -m pipeline.run --dataset <id>` turns a dataset's inputs (jazzPanda markers, the gene panel, an optional gene-set enrichment result, and viz sources) into a self-contained `data/datasets/<id>/` tree (`annotation.json`, `verdicts.csv`, per-cluster `interp/clusters/c{n}.json`, `gene_notes.json`, `celltype_notes.json`, and a `manifest.json` with provenance and artifact hashes) that the UI reads with no live recomputation. It runs in two skill-driven tiers:
...
- **Tier B, the skill's literature interpretation, live** (SKILL.md sits in the agent's system prompt): the Output-4 per-marker biology note is *the skill reading that gene's own Tier-A evidence*, its role framed by canonical status, with a specificity caveat flagged only when the numbers show the gene marks another lineage. One real live PMID or none.
...
On a compact three-cluster subset of the demo panel, the skill assigns c1 to *HER2-positive tumor epithelial*, c2 to *cancer-associated fibroblast*, and c3 to *macrophage*, with panel-absence evaluated against that dataset's own panel, a full run in seconds.
...
The demo dataset is the public **10x Genomics Xenium FFPE Human Breast Cancer (Rep 1)** sample: the 280-gene analyzed panel, 9 clusters (c1 to c9). Marker statistics come from **published jazzPanda** run offline; the app reads that **precomputed output** and never runs jazzPanda live. All spatial views (cell map, hex-bin density, UMAP) are built from the same public data. This keeps the demo deterministic and reproducible, and keeps the confident floor intact: every number on screen has a checkable source.

The Seurat object and jazzPanda results used here come from the jazzPanda paper: code at [phipsonlab/jazzPanda_paper](https://github.com/phipsonlab/jazzPanda_paper) and the data, including the Seurat object, at [Zenodo record 18149456](https://zenodo.org/records/18149456).
