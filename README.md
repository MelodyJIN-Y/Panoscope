...
8. **A second opinion, in two honest layers.** A deterministic **risk flag** (`agent/skeptic.py`) screens every call from the numbers alone (evidence thinness, a driver that localizes as well elsewhere, a rival whose on-panel markers also peak here) and reads *withstands* or *re-check* instantly; it auto-shows on a `verify`-flagged call as a triage light. It is a grounded rule, not an agent, and it is named as one. On demand, **"Pressure-test this call"** runs the live **second-opinion agent** (`loop.pressure_test`): it argues against the call *as it currently stands* (honoring any override), through the SAME tool loop and grounding gate as every other answer, adds one real live citation, and falls back to the deterministic report if the model is offline or its answer fails the floor. A careful adversarial contract keeps it from nonsense: it may never treat an off-panel absence as evidence, invent a rival with no on-panel signal, or manufacture doubt on a clean call. Asserted in `tests/test_skeptic.py`.
