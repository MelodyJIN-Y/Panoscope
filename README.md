...
7. **A downloadable interpretation summary.** The Summary page auto-assembles a per-dataset report from the durable, grounded artifacts: each cluster's call, confidence, driving numbers, the "what would settle it" line for shaky calls, the live-cited cell-type biology, and your saved lab notes with their tension. It is reviewable on the page and downloadable as Word (`.docx`) or PDF (`ui/report.py`). There is no live network at export time, so the report is a reproducible artifact. Asserted in `tests/test_report.py`.
