...
| **Second opinion** (risk flag + rival-test) | A deterministic *risk flag* screens every call from the numbers (thinness + localization + competing markers), auto on verify; the biologist tests a *specific* rival cell type from the chat ("Test another cell type") | Both trace to jazzPanda and clear the same grounding gate; the rival-test runs the grounded discriminator (`discriminate_call`) | [`agent/skeptic.py`](agent/skeptic.py), [`agent/discriminate.py`](agent/discriminate.py) |
